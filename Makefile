...
ifeq ($(PLATFORM), Darwin)
	...
	FRAMEWORKS =  -framework OpenGL -framework GLUT -framework IOKit -framework CoreFoundation
...
HIDAPI_SOURCES = hid.c
HIDAPI_OBJECTS = hid.o
...
simulator:	$(COMMON_OBJECTS) $(SIMULATOR_OBJECTS) $(HIDAPI_OBJECTS)
	$(CXX) -o $(SIMPLE_SIMULATOR) $+ $(SDL_LDFLAGS) $(FRAMEWORKS)

clean:
	rm -rf $(TUIO_DUMP) $(TUIO_DEMO) $(SIMPLE_SIMULATOR) $(TUIO_STATIC) $(TUIO_SHARED) $(COMMON_OBJECTS) $(DUMP_OBJECTS) $(DEMO_OBJECTS) $(SIMULATOR_OBJECTS) $(HIDAPI_OBJECTS)
